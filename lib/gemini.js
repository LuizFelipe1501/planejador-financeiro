...
export async function parseExpense(text) {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({
      ...
      generationConfig: {
        // temperature/top_p/top_k foram deprecados a partir do Gemini 3.x;
        // não enviar (a API pode retornar erro). Structured output basta.
...
// ---- Reconhecimento de PRINT de extrato/fatura (visão) ----
const IMAGE_SCHEMA = {
  type: 'object',
  properties: {
    expenses: {
      ...
      items: {
        type: 'object',
        properties: {
          amount: { type: 'number', description: 'valor do gasto em reais, positivo' },
          category: { type: 'string', enum: CATEGORIES },
          description: { type: 'string', description: 'estabelecimento ou item, curto' },
          occurred_at: { type: 'string', description: 'data YYYY-MM-DD' },
        },
        required: ['amount'],
      },
    },
...
// Recebe uma imagem (base64) de extrato/fatura e devolve uma LISTA de gastos.
export async function parseExpensesFromImage(base64, mimeType) {
  ...
  const prompt = [
    `Hoje é ${today} (fuso de Brasília).`,
    'A imagem é uma captura de tela ou foto de um extrato/fatura de banco ou app financeiro.',
    'Extraia TODOS os lançamentos de DÉBITO (gastos, compras, pagamentos, saídas de dinheiro).',
    'IGNORE entradas, créditos, recebimentos, transferências recebidas, saldos e totais.',
    'Para cada gasto: amount = valor positivo em reais (número); category = uma das categorias;',
    'description = estabelecimento/curto; occurred_at = data YYYY-MM-DD (se só houver dia/mês, use o ano atual; sem data, use hoje).',
    'Se não houver nenhum gasto reconhecível, retorne expenses = [].',
  ].join('\n');
  ...
  if (!res.ok) {
    const detail = await res.text();
    throw new Error(`Gemini visão ${res.status}: ${detail}`);
  }
